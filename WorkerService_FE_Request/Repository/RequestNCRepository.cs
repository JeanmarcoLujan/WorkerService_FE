...
namespace WorkerService_FE_Request.Repository
{
    public class RequestNCRepository : IRequestNCRepository
    {
        ...
        public int GetDocumentSAPNC()
        {
            ...
            var asdasd = dataTable.AsEnumerable().Count();
            var asdasds = dataTable.AsEnumerable();
                        ...
                        using (HttpClient httpClient = new HttpClient())
                        {
                            ...
                            try
                            {
                                ...
                                if (result1.IsSuccessStatusCode)
                                {
                                    ...
                                    var asdasd = Path.Combine(pathOut, fileName);
                                    var asdasd1 = Path.Combine(path, fileName);
                                    System.IO.File.Move(path + fileName, Path.Combine(pathOut, fileName));
